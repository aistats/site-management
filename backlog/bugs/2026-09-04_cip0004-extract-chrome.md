...
- [ ] `main_after_nav` (and shared helpers) remove nav/header/footer, cookie banners, empty dropdown shells, and redundant Bootstrap wrappers before pandoc
- [ ] Converted Call for Papers fixture or live snapshot has no `child-menu` / nested empty `container-fluid` lead-in; key virtual sentences still present
- [ ] `registration_blocks`, `schedule_summary`, and `dates_tables` produce clearly different bodies (or registration/schedule stay link-heavy summaries rather than full Dates dumps)
- [ ] `event_bios` keeps speaker/award names and bio text; drops bookmark/badge/card chrome where practical
- [ ] Unit/fixture tests cover chrome strip and at least one Dates-strategy divergence
- [ ] Re-convert spot-check: CFP and invited candidates reviewed against virtual wording (faithful, not paraphrased)
